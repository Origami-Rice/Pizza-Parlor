...
def setup_pizza():
    ...
    new_item = Pizza(pizza, int(size), 1)
    return new_item
...
def add_pizza_to_order(order):
    newItem = setup_pizza()
    additional_toppings = input(
        "Do you want additional toppings (y/n)? ")
    if (additional_toppings == "y"):
        additional_toppings = setup_topping()
        newItem.addToppings(additional_toppings)
...
def handle_add_item_request(order):
    print('''which item do you want: 
                1. Pizza.
                2. Drinks.  ''')
    item_choice = input("Make your Choice: ")
    if item_choice == "1":
...
def handle_item_type_update(item, new_item):
    typeCheck = input('''Do you want to change the Type?
                        yes or no: ''')
    if typeCheck == "yes":
        newType = setup_type(item["category"])
        new_item.changeType(newType)
...
def uber_delivery(order, order_number, address):
    ...
    json_delivery = get_delivery_as_Json(order, order_number, address)
    print(str(json_delivery))
